...
def numpy_sepia_filter(image, intensity=None):
    ...
    Args:
        ...
        intensity (float): The level of intensity of the sepia image (from a scale of 0 to 1)
    Returns:
        sepia_image (numpy.ndarray): A sepia version of the image.
    """
    if intensity or intensity==0:
        sepia_matrix = np.array(
            [[1+intensity*(0.131-1), 0.534*intensity, 0.272*intensity], 
            [0.168*intensity, 1+(intensity*(0.686-1)), 0.349*intensity], 
            [0.189*intensity, 0.769*intensity, 1+intensity*(0.393-1)]]
        )
    else:
        sepia_matrix = np.array(
